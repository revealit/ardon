...
var Migration = module.exports = function (options) {
  ...
  // Purge the database of all objects created by Ardon.
  // This is usually used for resetting the database before re-running
  // all the migrations. A shortcut for doing this is setting the
  // migrate option to true. 
  self.purgeDatabase = function (callback, migrate) {
    var migrate = migrate;
    self.runSQL(['purge.sql'], function (err) {
      if (err) {
        throw err;
      }

      if (migrate) {
        self.startMigration(callback);
      }
      else {
        callback();
      }
